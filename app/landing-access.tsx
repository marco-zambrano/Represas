...
export function LandingAccess() {
  ...
  return <section className="panel flex min-h-[595px] flex-col p-6 sm:p-8">
    <p className="eyebrow">Tu acceso al tablero</p>
    <h2 className="mt-3 min-h-[72px] text-3xl font-black">{mode === "login" ? "Ingresa a HidroVista" : "Crea tu cuenta"}</h2>
    <p className="mt-2 min-h-12 max-w-sm text-sm leading-6 text-[#527174]">{mode === "login" ? "Consulta el monitoreo de las centrales en un solo lugar." : "Te enviaremos un correo de confirmación para activar el acceso."}</p>
    ...
    <div className="mt-4 h-6">{mode === "login" && <Link href="/forgot-password" className="inline-block text-sm font-bold text-[#007c8d]">Olvidé mi contraseña</Link>}</div>
